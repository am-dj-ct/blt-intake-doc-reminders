...
function loadBroker(env = process.env, deps = {}) {
  ...
  for (const [name, value] of [
    ...
    // Carries the identity read's specific failure reason
    // (identity_element_missing / identity_element_empty / identity_read_error)
    // instead of the collapsed generic "identity_unreadable" the plain
    // readLoggedInUsername wrapper leaves the caller no way to distinguish.
    ["readLoggedInIdentity", schedule?.readLoggedInIdentity],
    // Required, not optional: without it this repo cannot release the intent
    // marker it writes before every login, and a job that cannot release its
    // marker is a job that leaks one every hour. Verified present in the
    // pinned runtime this repo attests against.
    ["confirmAccountBrokerLoginUsable", schedule?.confirmAccountBrokerLoginUsable],
