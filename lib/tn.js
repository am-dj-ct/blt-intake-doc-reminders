...
// Returns { ok: true } once the day grid has visibly rendered, or
// { ok: false, error } if the render never showed up in time. Callers that
// treat an empty scrape as proof-of-emptiness (the digest gate) must check
// `ok` first — a load failure and a genuinely empty day both scrape as [],
// and only one of those is proof of anything.
