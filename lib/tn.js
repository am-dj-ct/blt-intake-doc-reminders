...
// Self-hosted CI can keep this Mac above load 80; Chrome has exceeded the
// former two-minute launch deadline while the same account stayed healthy.
const BROWSER_LAUNCH_TIMEOUT_MS = 300_000;
...
// ---- browser ----
async function launch({ headless = true, profileDir } = {}) {
  if (!profileDir) throw new Error('A broker-selected TherapyNotes profile is required.');
  let context;
  try {
    context = await chromium.launchPersistentContext(profileDir, {
      ...
      viewport: { width: 2600, height: 1100 },
      args: ['--disable-blink-features=AutomationControlled'],
      ignoreDefaultArgs: ['--enable-automation'],
    });
  } catch (error) {
    if (/^browserType\.launchPersistentContext: Timeout \d+ms exceeded\./.test(String(error?.message || ''))) {
      error.code = 'tn_browser_launch_timeout';
      error.alertCode = 'tn_browser_launch_timeout';
      error.stage = 'browser_launch';
      error.timeoutMs = BROWSER_LAUNCH_TIMEOUT_MS;
    }
    throw error;
  }
...
// Returns { ok, coverageOk, error? }.
//   ok         — the day grid itself visibly rendered (clinician columns
//                showed up in time). Appointment bubbles are deliberately not
//                used as the signal: a legitimately empty day has none.
//   coverageOk — resetCalendarViewIfFiltered() reports the clinician column
//                count is back to the full roster (>= MIN_CLINICIAN_COLUMNS).
//                false means the day view may still be filtered to a subset
//                of clinicians, so an empty scrape is NOT proof every
//                clinician's schedule was checked.
// Callers that treat an empty scrape as proof-of-emptiness (the digest gate)
// must check both before trusting a zero count.
async function gotoDay(page, dateStr) {
  ...
  const headers = await resetCalendarViewIfFiltered(page);
  const coverageOk = Array.isArray(headers) && headers.length >= MIN_CLINICIAN_COLUMNS;
  try {
    await page.waitForSelector('.calendar-weekview-dayColumn', { timeout: 15000 });
    return { ok: true, coverageOk };
  } catch (error) {
    return { ok: false, coverageOk, error };
  }
}
...
async function getDocumentTitles(page, patientId) {
  ...
  // Keep the original fixed pause as a floor (the header row can render
  // before the document rows), then wait up to 20 s more for the table or
  // TN's explicit "None". On timeout, fall through exactly as before.
  await page.waitForTimeout(3500);
  await page.waitForFunction(() => {
    const norm = t => (t || '').replace(/\s+/g, ' ').trim();
    if (/Notes and Documents for this Patient:\s*None/i.test(norm(document.body.innerText))) return true;
    return Array.from(document.querySelectorAll('table')).some(tbl => {
