...
// `opts.onSubmit`, when given, is invoked at the moment the password submit
// is clicked — the account-broker login-safety wrapper (see
// lib/tn-account-session.js) uses this to distinguish pre-submit failures
// (never arm a cooldown) from post-submit ones (arm the cooldown; TN may
// have received and rejected it). Legacy (flag-off) callers don't pass it.
async function login(page, creds = loadCreds(), opts = {}) {
  ...
  if (typeof opts.onSubmit === 'function') opts.onSubmit();
