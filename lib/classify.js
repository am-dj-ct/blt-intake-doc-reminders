...
async function classifyDocs(rows) {
  ...
  // Resolve the network SDK only when classification actually needs it. This
  // preserves the empty-list fast path and keeps non-inference consumers from
  // depending on a runtime-only package at import time.
