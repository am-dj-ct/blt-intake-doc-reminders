...
function confirm({ client, clinicianName, apptHuman, needsForwarding = false }) {
  return {
    ...
    html: p(
      ...
      needsForwarding ? `No therapist email is on file for "${clinicianName}", so front desk should forward this confirmation.` : null
    ) + TAG,
  };
}
