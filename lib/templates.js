...
// Daily digest / heartbeat -> jesse@. Lists tomorrow's virtual intakes and
// whether each has its SOD and GAINSS as of this run.
function digest({ ranAt, dateLabel, intakes }) {
  const n = intakes.length;
  const mark = b => (b ? '✓' : '✗');
  const body = n === 0
    ? '<p>No virtual intakes scheduled for tomorrow.</p>'
    : '<ul>' + intakes.map(i =>
        `<li>${i.time} — <b>${i.client}</b> (${i.clinician}) — SOD ${mark(i.hasSOD)} &middot; GAINSS ${mark(i.hasGAINSS)}</li>`
      ).join('') + '</ul>';
  return {
    subject: n === 0
      ? 'BLT intake check — no virtual intakes tomorrow'
      : `BLT intake check — ${n} virtual intake${n === 1 ? '' : 's'} tomorrow (${dateLabel})`,
    html: p(`Daily check — the intake reminder ran at ${ranAt} and is working.`)
      + `<p><b>Virtual intakes tomorrow (${dateLabel}):</b></p>`
      + body + TAG,
  };
}

module.exports = { nag, escalation, confirm, digest };
