...
async function assertIdentityOrThrow({ page, broker, resolved }) {
  // The pinned reader deliberately returns an empty string for any transient
  // Profile-link/navigation/DOM failure. A single empty read is therefore not
  // proof of a wrong account. Re-read the live profile a small, fixed number
  // of times, but never retry a readable mismatch: the identity boundary must
  // remain fail-closed when TN actually shows a different username.
  for (let attempt = 1; attempt <= IDENTITY_READ_ATTEMPTS; attempt += 1) {
    ...
    if (identity.reason !== "identity_unreadable" || attempt === IDENTITY_READ_ATTEMPTS) {
      throw new Error(`TN identity assertion failed for account "${resolved.account}": ${identity.reason}`);
    }
  }
  throw new Error(`TN identity assertion failed for account "${resolved.account}": identity_unreadable`);
}
