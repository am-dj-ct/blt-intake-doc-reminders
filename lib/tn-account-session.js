...
function securePathTree(profileDir, { lockOwnershipVerified = false } = {}) {
  ...
  // The account directory may contain broker-owned siblings such as
  // singleton-quarantine-<timestamp>. Those directories deliberately retain
  // Chrome's transient symlinks as incident evidence and are outside this
  // session's locked browser profile. Traversing the parent made a valid
  // quarantine sibling fail every later run as an "unapproved symlink".
  // The ancestry loop above already validates and secures each directory on
  // the path; recursively inspect only the profile this lock owns.
  visit(profileDir);
}
