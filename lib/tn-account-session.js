...
function securePathTree(profileDir) {
  ...
  const visit = (target) => {
    ...
    if (stat.isSymbolicLink() || (uid !== null && stat.uid !== uid)) throw new Error("TherapyNotes profile tree is not owner-controlled.");
    if (stat.isDirectory()) {
...
async function cleanupAndRelease({ launched, profileDir, lockSession, broker, timeoutMs = CLEANUP_TIMEOUT_MS }) {
  ...
  try { securePathTree(profileDir); } catch (error) { errors.push(error); }
  try {
