...
test("document lookup waits for a table or explicit empty state and falls through on timeout", async () => {
  for (const waitError of [null, new Error("synthetic timeout")]) {
    ...
    assert.match(calls[1][1], /Notes and Documents for this Patient/);
    assert.match(calls[1][1], /querySelectorAll\('table'\)/);
    assert.equal(calls[1][2], undefined);
    assert.deepEqual(calls[1][3], { timeout: 20000 });
    assert.deepEqual(calls[2], ["evaluate"]);
  }
});
